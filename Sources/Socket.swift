...
open class Socket: Hashable, Equatable {
    ...
    public func port() throws -> in_port_t {
        ...
        return try withUnsafePointer(to: &addr) { pointer in
            ...
            if getsockname(socketFileDescriptor, pointer.withMemoryRebound(to: sockaddr.self, capacity: 1, { $0 }), &len) != 0 {
    ...
    public func writeUInt8(_ data: ArraySlice<UInt8>) throws {
        try data.withUnsafeBufferPointer {
            try writeBuffer(UnsafeRawPointer($0.baseAddress)!, length: data.count)
        }
    }

    public func writeData(_ data: Data) throws {
        let bytes = data.withUnsafeBytes { UnsafeRawPointer($0) }
        try writeBuffer(bytes, length: data.count)
    }
